...
export default function AddReaction(props: Props): JSX.Element {
  ...
  const addReaction = async (emoji: BaseEmoji) => {
    ...
    const response = await axios.post(`${API_URL}/v1/memes/${props.meme.id}/reactions`, addMemeReactionInput);

    const nextReactions = reactions ? [...reactions] : [];

    if (response.data) {
      nextReactions.push(response.data);
    }

    dispatch(setReactions(nextReactions));
  };
