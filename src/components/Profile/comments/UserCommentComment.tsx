...
import { GodAction, GodComment, GodMeme } from "../../../types";
...
export default function UserCommentComment(props: Props): JSX.Element | null {
  ...
  useEffect(() => {
    async function getGodCommentAsync(commentId: string) {
      const godComment = await fetchGodComment(commentId);
      setParentComment(godComment);
    }

    if (props.action.comment?.id) {
      getGodCommentAsync(props.action.comment.id);
    }
  }, [props.action]);

  if (!parentMeme || !parentComment || !props.action.user) return null;
    ...
    >
      <Grid style={{ display: "flex", flexDirection: "row", padding: 16 }}>
        <Typography variant="body2">
          ...
          replied to
          <span style={{ fontWeight: "bold", marginLeft: 4 }}>{`${parentComment?.user.name}`}</span>
          {`'s comment on`}
