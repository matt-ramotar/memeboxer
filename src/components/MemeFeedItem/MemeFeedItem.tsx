...
export interface UserIdToMemeReactionIdsMap {
    [userId: string]: ReactionIdToUserIdsMap;
}

export interface ReactionIdToMemeReactionIdsMap {
    [reactionId: string]: string[];
}
    ...
    useEffect(() => {
        if (memeReactionsRedux) {
            ...
            for (const memeReaction of memeReactionsRedux) {
                ...
                if (userIdToMemeReactionIdsMap[memeReaction.userId]) {
                    if (userIdToMemeReactionIdsMap[memeReaction.userId][memeReaction.reactionId]) {
                        userIdToMemeReactionIdsMap[memeReaction.userId][memeReaction.reactionId].push(memeReaction.id);
                    } else {
                        userIdToMemeReactionIdsMap[memeReaction.userId][memeReaction.reactionId] = [memeReaction.id];
                    }
                } else {
                    userIdToMemeReactionIdsMap[memeReaction.userId] = {
                        [memeReaction.reactionId]: [memeReaction.id],
                    };
                }
            }
            ...
            <Box style={{ border: `1px solid ${theme.palette.divider}`, paddingTop: 8, paddingBottom: 8 }}>
                ...
                <Box style={{ display: "flex", flexDirection: "column", justifyContent: "flex-start", alignItems: "space-around", paddingTop: 8, width: "100%" }}>
                    ...
                    <Grid
                        container
                        style={{
                            display: memeReactionInfo.commenters.length > 0 ? "flex" : "none",
                            flexDirection: "row",
                            alignItems: "center",
                            cursor: "pointer",
                            marginTop: 12,
