...
import { CreateMemeInput } from "../../lib/meme";
import { RootState } from "../../store";
import { clearComponents, clearTagInfo, setActiveComponent, setCurrentJob, setData, setTextList } from "../../store/createMeme";
...
export default function CreateMemeFlow(): JSX.Element {
    ...
    const memeTagInputsRedux = useSelector((state: RootState) => state.createMeme.memeTagInputs);
    ...
    const createMemeAsync = async (data: string, templateId: string, userId: string) => {
        const input: CreateMemeInput = {
            data,
            templateId,
            userId,
            ...
            memeTagInputs: memeTagInputsRedux ?? undefined,
        };
    ...
    const onNext = () => {
        if (currentJob == 1) dispatch(setCurrentJob(2));
            ...
            if (dataRedux && templateIdRedux && userRedux && userRedux.id) {
                createMemeAsync(dataRedux, templateIdRedux, userRedux.id);
            }
        }
    };
            ...
            >
                ...
                >
                    <button
                        ...
                        onClick={() => {
                            if (currentJob == 3) dispatch(clearTagInfo());
